...
die "Expected 5 arguments, got "
  ...
  if !$opts{mandir} || $ARGV[8];
die
"Strange Perl installation, no \@INC! Can't install Perl-Module(s), killing myself..\n"
  if !$pmdir;
...
sub extract_man {
    ...
    foreach ( glob($glob) ) {
        ...
        system( $opts{podmanbin}, "--center", "User commands",
            "$_", "$dest/$file.1"
        );
...
sub remove_docs {
    ...
    if (
        system( "install-info", "--dir-file=$infodir/dir", "--delete", $file ) )
    {
        print
" > Could not remove documentation, maybe you didn't install the docs ;)\n";
    }
    else {
...
sub _recmkdir {
    ...
    foreach ( split( /\//, $dir ) ) {
        ...
        next if -e $step;
        mkdir( $step, 0755 )
          or die "_recmkdir($dir): Failed to create $step: $!\n";
    }
    return $step;
}
