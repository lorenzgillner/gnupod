...
$AWDB->LoadArtworkDb;
GNUpod::XMLhelper::doxml( $connection->{xml} )
  or usage("Failed to parse $connection->{xml}, did you run gnupod-init?\n");
...
sub newfile {
    ...
    if ( exists( $el->{file}->{dbid_1} ) ) {
        ...
        foreach my $awobj ( @{ $awref->{subimages} } ) {
            if ( $opts{unique}
                && defined( $unique{ $awobj->{path} . $awobj->{offset} } ) )
            {
                next;
            }
            else {
...
"$artist - $album - $title ($awobj->{width}x$awobj->{height}).bmp";
            ...
            print
"Extracting $awobj->{width}x$awobj->{height} version of $artist - $album - $title\n";
            open( ITHMB, "<", $awdb_path )
              or die "Unable to open $awdb_path: $!\n";
            binmode(ITHMB);
            seek( ITHMB, $awobj->{offset}, 0 )
              or die "seek($awobj->{offset}) failed: $!\n";
