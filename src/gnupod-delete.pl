...
sub main {
    ...
    if ( @resultlist or %playlist_names ) {
        ...
        if ( $opts{force} ) {
            ...
            or ( scalar(@resultlist) > $max_non_interactive_delete ) )
        {
...
sub newfile {
    ...
    if ($firstrun) {
        ...
        if ( $resultids{ $el->{file}{id} } ) {
            ...
            unlink(
                GNUpod::XMLhelper::realpath(
                    $opts{mount}, $el->{file}->{path}
                )
...
sub newpl {
    ...
    if ($firstrun) {
        ...
        elsif ( $plt eq "spl" && ref( $el->{splcont} ) eq "HASH" )
        {              #spl content
