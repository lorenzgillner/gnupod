...
GNUpod::FooBar::GetConfig( \%opts,
    { mount => 's', force => 'b', model => 's', low_ram_attr => 's' },
    "tunes2pod" );
...
sub convert {
    ...
    if ( $opts{'low_ram_attr'} ) {
        print
"> Parsing GNUtunesDB.xml document to prepare merge of limited attributes...\n";
        GNUpod::XMLhelper::doxml( $con->{xml} )
          or usage("Could not read $con->{xml}, did you run gnupod-init ?");
        GNUpod::XMLhelper::resetxml();
        print "\r> "
          . $xml_files_parsed
          . " files parsed, converting iTunesDB...\n";
    }

    open( ITUNES, $con->{itunesdb} )
      or usage("Could not open $con->{itunesdb}");

    while (<ITUNES>) { };
    sysseek( ITUNES, 0, 0 )
      ;  # the iPod is a sloooow mass-storage device, slurp it into the fs-cache
    ...
    my $obj = {
        ...
        callback => {
            PACKAGE => $self,
            mhod    => { item  => 'MhodItem' },
            mhit    => { start => 'MhitStart', end => 'MhitEnd' },
            mhsd    => { start => 'MhsdStart' },
            mhip    => { item  => 'MhipItem' },
            mhyp    => { item  => 'MhypItem', end => 'MhypEnd' },
            mhlt    => { item  => 'MhltItem' },
        }
    };
    ...
    GNUpod::FooBar::SetItunesDBAsInSync($con)
      ;    # GNUtunesDB.xml is in-sync with iTunesDB
    ...
    print "\n Done\n";
    close(ITUNES)
      or die "Failed to close filehandle of $con->{itunesdb} : $!\n";
    exit(0);
}
...
sub SetPodcastIndex {
    ...
    $self->{pc_playlist}->{lists}->{$index} =
      { name => 'Lost and Found', content => [] };
}
...
sub SetPodcastName {
    ...
    return if $index == 0;
    $self->{pc_playlist}->{lists}->{$index}->{name} = $name
      if length($name) != 0;
}
...
sub MhsdStart {
    ...
    if ( $old == MODE_SONGS ) {
        print
"\r> $self->{count_songs_done} of $self->{count_songs_total} files found, searching playlists\n";
    }
}
...
sub MhitEnd {
    ...
    if ( $self->{mode} == MODE_SONGS ) {
        ...
        if ( $i % 32 == 0 ) {
            printf(
                "\r> %d files left, %d%% done    ",
                $self->{count_songs_total} - $i,
                ( $i / ( 1 + $self->{count_songs_total} ) * 100 )
            );
        }
    }
    else {
...
sub MhodItem {
    ...
    if ( $self->{mode} == MODE_SONGS ) {
        ...
        # Legacy playlist
        if ( defined( $args{ref}->{type_string} )
            && ( $args{ref}->{type_string} eq 'title' ) )
        {
...
sub MhipItem {
    ...
    if ( $self->{mode} == MODE_OLDPL ) {
        ...
        if (   $args{ref}->{podcast_group} == 256
            && $args{ref}->{podcast_group_ref} == 0 )
        {
            # -> Podcast index found
            $self->SetPodcastIndex( $args{ref}->{plid} );
        }
        elsif ($args{ref}->{podcast_group} == 0
            && $args{ref}->{podcast_group_ref} != 0 )
        {
            # -> New item for an index found, add it
            $self->AppendPodcastItem( $args{ref}->{podcast_group_ref},
                $args{ref}->{sid} );
        }
    }
}
...
sub MhypEnd {
    my ( $self, %args ) = @_;
    if ( $self->{mode} == MODE_OLDPL ) {
        if (   $self->{playlist}->{mpl} == 0
            && $self->{playlist}->{podcast} == 0 )
        {
            # -> 'Old' non-podcast playlist
            my $plname = $self->{playlist}->{name};

            if (   ref( $self->{playlist}->{spl}->{preferences} ) eq "HASH"
                && ref( $self->{playlist}->{spl}->{data} ) eq "ARRAY" )
            {
                ...
                my $ns   = 0;
                my $nr   = 0;
                GNUpod::XMLhelper::addspl(
                    $plname,
                    {
                        liveupdate => $pref->{live},
                        moselected => $pref->{mos},
                        limititem  => $pref->{iitem},
                        limitsort  => $pref->{isort},
                        limitval   => $pref->{value},
                        matchany   => $self->{playlist}->{spl}->{matchrule},
                        checkrule  => $pref->{checkrule},
                        plid       => $self->{playlist}->{plid}
                    }
                );
                foreach my $splitem ( @{ $self->{playlist}->{spl}->{data} } ) {
                    GNUpod::XMLhelper::mkfile( { spl => $splitem },
                        { splname => $plname } );
                    $nr++;
                }
                foreach my $id ( @{ $self->{playlist}->{content} } ) {
                    GNUpod::XMLhelper::mkfile(
                        { splcont => { id => $id } },
                        { splname => $plname }
                    );
                    $ns++;
                ...
                my $ns = 0;
                GNUpod::XMLhelper::addpl( $plname,
                    { plid => $self->{playlist}->{plid} } );
                foreach my $id ( @{ $self->{playlist}->{content} } ) {
                    GNUpod::XMLhelper::mkfile( { add => { id => $id } },
                        { plname => $self->{playlist}->{name} } );
                    $ns++;
                }
                print " with $ns songs\n";
            }
        ...
        foreach my $pci ( sort keys( %{ $self->{pc_playlist}->{lists} } ) ) {
            ...
            foreach my $i ( @{ $cl->{content} } ) {
                GNUpod::XMLhelper::mkfile( { add => { id => $i } },
                    { plname => $cl->{name} } );
                $ns++;
            }
            print " with $ns songs\n";
...
sub newfile {
    ...
    $xml_files_parsed++;
    print "\r> " . $xml_files_parsed . " files parsed"
      if $xml_files_parsed % 96 == 0;
