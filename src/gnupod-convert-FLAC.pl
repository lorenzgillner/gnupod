...
if ( !( -r $file ) ) {
    ...
    foreach my $flac_pmod ( 'Audio::FLAC::Header', 'Audio::FLAC' ) {
        my $nocompile =
          "use $flac_pmod; \$ftag = $flac_pmod->new( \$file )->tags();";
    ...
    open( FLACOUT, "-|" )
      or exec( "flac", "-d", "-s", "-c", "$file" )
      or die "Could not exec flac: $!\n";
    open( LAMEIN, "|-" )
    ...
    check_ffmpeg_alac()
      or die "ffmpeg not found or ffmpeg does not support ALAC encoding\n";
    my $tmpout = GNUpod::FooBar::get_u_path( "/tmp/gnupod_alac", "m4a" );
    my $status =
      system( "ffmpeg", "-i", "$file", "-acodec", "alac", "-v", "0", $tmpout );
    if ($status) {
