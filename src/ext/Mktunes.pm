...
sub WriteItunesSD {
    my ($self) = @_;
    open( ITS, ">", $self->GetConnection->{itunessd} )
      or die
      "*** Unable to write the iTunesDB: $!, did you run gnupod-init ?\n";
    binmode(ITS);
    print ITS GNUpod::iTunesDB::mk_itunes_sd_header(
        { files => $self->GetFileCount } );
    foreach my $item ( @{ $self->GetFiles } ) {
        print ITS GNUpod::iTunesDB::mk_itunes_sd_file($item);
    }
    close(ITS)
      or die "Failed to close filehandle of "
      . $self->GetConnection->{itunessd}
      . " : $!\n";
}
    ...
    print ITUNES GNUpod::iTunesDB::mk_mhsd(
        {
            type => 3,
            size => length(
                GNUpod::iTunesDB::mk_mhlp( {} ) . $playlists->{newstyle}
            )
        }
    );
    print ITUNES GNUpod::iTunesDB::mk_mhlp(
        { playlists => $playlists->{count_newstyle} } );
    ...
    print ITUNES GNUpod::iTunesDB::mk_mhsd(
        {
            type => 2,
            size =>
              length( GNUpod::iTunesDB::mk_mhlp( {} ) . $playlists->{legacy} )
        }
    );
    print ITUNES GNUpod::iTunesDB::mk_mhlp(
        { playlists => $playlists->{count_legacy} } );
    print ITUNES $playlists->{legacy};
    $mhbd_size = tell(ITUNES) - $mhbd_size;

    # Fixup some things:
    GNUpod::FooBar::SeekFix( *ITUNES, 0,
        GNUpod::iTunesDB::mk_mhbd( { size => $mhbd_size, childs => 3 } ) );
    GNUpod::FooBar::SeekFix( *ITUNES, $mhsd_pos,
        GNUpod::iTunesDB::mk_mhsd( { size => $mhsd_size, type => 1 } ) );
    close(ITUNES) or die "Failed to close filehandle of $tmpfile : $!\n";

    unlink($outfile);    # can fail
    rename( $tmpfile, $outfile )
      or die "*** Unable to move $tmpfile to $outfile : $!\n";
}
...
sub GetSmartPlaylist {
    my ( $self, $name ) = @_;
    return ( $self->{SmartPlaylists}->{$name} || [] );
}
...
sub CreateAllPlaylists {
    my ($self) = @_;
    my $mpl_name =
      Unicode::String::utf8( $self->{iPodName} || IPODNAME_FALLBACK )->utf8;
    my $master_playlist = $self->CreateSinglePlaylist(
        Name       => $mpl_name,
        Type       => PLAYLIST_HIDDEN,
        Content    => $self->GetMasterPlaylist,
        PlaylistId => MPL_UID
    )->{payload};
    ...
    my $playlist_count_o = 1
      ; # Old/Legacy playlist count: MasterPlaylist + N-Other                         = 1+x
    my $playlist_count_n = 2
      ; # New playlist count       : MasterPlaylist + PodcastPlaylistHeader + N-Other = 2+x
    my $podcast_id = 1;    # First podcast-id to use
    my @podcast_playlist =
      ();    # Holds items we are going to slap into the legacy podcast playlist
    my $podcast_childs = 0;

    foreach my $plref ( GNUpod::XMLhelper::getpl_attribs() ) {
        my $splh =
          GNUpod::XMLhelper::get_splpref( $plref->{name} );    #Get SPL Prefs
        my $plh = GNUpod::XMLhelper::get_plpref( $plref->{name} )
          ;    #Get normal-pl preferences
        ...
        if ( $plh->{podcast} == 1 ) {
            ...
            push( @podcast_playlist,
                @{ $self->GetPlaylist( $plref->{name} ) } );

            # Create the newstyle-podcast chunk
            $playlist = $self->CreateSinglePlaylist(
                Name           => $plref->{name},
                Type           => PLAYLIST_VISIBLE,
                PodcastContent => $self->GetPlaylist( $plref->{name} ),
                PlaylistId     => $plref->{plid},
                Sortby         => $plref->{sort},
                Podcast        => 1,
            ...
            $podcast_id +=
              $playlist->{childs};    # Set next podcast id (iTunes way..)
        }
        else {
            # Normal playlist:
            $playlist = $self->CreateSinglePlaylist(
                Name       => $plref->{name},
                Type       => PLAYLIST_VISIBLE,
                Content    => $self->GetPlaylist( $plref->{name} ),
                PlaylistId => $plref->{plid},
                SplContent => $splh,
                Sortby     => $plref->{sort}
            );
        ...
        my $pl_type =
          ( $playlist->{smartplaylist}
            ? 'Smart-'
            : ( $playlist->{podcastplaylist} ? 'Podcast-' : '' ) );

        print ">> Created " . $pl_type
          . "Playlist '$plref->{name}' with $playlist->{songs} files\n";
    }

    if ($podcast_childs) {

        # -> We need to create a legacy list:
        $legacy_pc_buff = $self->CreateSinglePlaylist(
            Name    => 'Podcasts',
            Type    => PLAYLIST_VISIBLE,
            Content => \@podcast_playlist,
            Podcast => 1,
            Sortby  => 'releasedate'
        )->{payload};
        $playlist_count_o++;
    ...
    $new_pc_buff = GNUpod::iTunesDB::mk_mhyp(
        {
            size    => length($new_pc_buff),
            name    => 'Podcasts',
            type    => 0,
            files   => $podcast_childs,
            podcast => 1
        }
    ) . $new_pc_buff;

    return {
        legacy         => $master_playlist . $playlist_buff . $legacy_pc_buff,
        count_legacy   => $playlist_count_o,
        newstyle       => $master_playlist . $playlist_buff . $new_pc_buff,
        count_newstyle => $playlist_count_n
    };
}
...
sub CreateSinglePlaylist {
    ...
    my $pcast =
      $args{Podcast};    # Mark playlist as podcast (used by new and oldstyle)
    my $pcont = $args{PodcastContent}
      ;    # Content for newstyle podcasts (also needs PodcastId to be non-null)
    my $pcid         = $args{PodcastId};       # Podcast id to use
    my $seperate_hdr = $args{SeperateHeader}
      ; # Do not include the header in our payload but drop it into the 'header' field
    my $buff_playlist  = '';    # Payload buffer
    my $mhod_count     = 0;     # Mhods we created
    my $spl_mhod_count = 0;     # SmartPlaylist mhods
    my $child_count =
      0;    # Childs we created (may be == mhod_count if no spl is there)
    ...
    if ($pcid) {
        ...
        my $pcplid = 1 + $pcid;
        my $current_mhod =
          GNUpod::iTunesDB::mk_mhod( { stype => 'title', string => $name } );
        my $current_mhip = GNUpod::iTunesDB::mk_mhip(
            {
                childs        => 1,
                podcast_group => 256,
                plid          => $pcid,
        ...
        foreach my $fqid ( @{$pcont} ) {
            ...
            my $x_mhip = GNUpod::iTunesDB::mk_mhip(
                {
                    childs            => 1,
                    sid               => $fqid,
                    plid              => $pcplid,
                    podcast_group_ref => $pcid,
                    size              => length($x_mhod)
                }
            );
        ...
        if ( ref($cspl) eq "HASH" ) {
            ...
            $buff_playlist .= GNUpod::iTunesDB::mk_splprefmhod(
                {
                    item       => $cspl->{limititem},
                    sort       => $cspl->{limitsort},
                    mos        => $cspl->{moselected},
                    liveupdate => $cspl->{liveupdate},
                    value      => $cspl->{limitval},
                    checkrule  => $cspl->{checkrule}
                }
            ) || die "Failed to create splprefmhod\n";

            $buff_playlist .= GNUpod::iTunesDB::mk_spldatamhod(
                {
                    anymatch => $cspl->{matchany},
                    data     => $self->GetSmartPlaylist($name)
                }
            ) || die "Failed to create spldatamhod\n";
            $mhod_count += 2;
            $spl_mhod_count++;
        }

        foreach my $fqid ( @{$cont} ) {
            if ( !$self->{SequenceJumped} ) {
                my $i = 1 << 15;
                while ( $i < $self->{Sequence} ) { $i <<= 1; };
                $self->{Sequence}       = $i;
                $self->{SequenceJumped} = 1;
            }
    ...
    my $mhyp = GNUpod::iTunesDB::mk_mhyp(
        {
            size        => length($buff_playlist),
            name        => $name,
            type        => $type,
            files       => $child_count,
            stringmhods => $spl_mhod_count,
            mhods       => $mhod_count,
            plid        => $plid,
            podcast     => $pcast
        }
    ...
    return (
        {
            header          => $mhyp,
            payload         => $buff_playlist,
            childs          => $child_count,
            mhods           => $mhod_count,
            songs           => $songs_count,
            smartplaylist   => int( $spl_mhod_count != 0 ),
            podcastplaylist => $podcast_list
        }
    );
...
sub AssembleMhit {
    ...
    foreach my $key ( sort keys(%$object) ) {
        ...
        next
          if ( scalar keys %$keep && !$keep->{$key} )
          ;                            # Only keep specific mhods
        my $new_mhod =
          GNUpod::iTunesDB::mk_mhod( { stype => $key, string => $value } );
        next unless $new_mhod;         # Something went wrong
        $mhod_chunks .= $new_mhod;
        $mhod_count++;
    }
    $mhit = GNUpod::iTunesDB::mk_mhit(
        {
            size    => length($mhod_chunks),
            count   => $mhod_count,
            fh      => $object,
...
sub AddFile {
    ...
    $item->{id} = $current_id
      ;    # The iTunesDB will use the sequence id, not the id from the XML file
...
"mktunes.pl: warning: (pl: $name) Liveupdate disabled. Please set liveupdate=\"1\" if you don't want an empty playlist\n";
    ...
    { #We found an old id with disabled liveupdate, add it like a normal playlist:
        foreach ( sort { $a <=> $b }
            split( / /, $self->{FuzzyDb_Normal}->{id}->{$id} ) )
        {
            $self->_AddToPlaylist( Name => $name, Id => $_ );
        }
    }
}
        ...
        if ( $action eq "add" ) {
            ...
            foreach my $xrn ( keys( %{ $item->{$action} } ) ) {
                ...
                foreach my $val ( keys( %{ $self->{FuzzyDb_Normal}->{$xrn} } ) )
                {
                    ...
                    ##get the keys
                    foreach (
                        split( / /, $self->{FuzzyDb_Normal}->{$xrn}->{$mval} ) )
                    {
                        $mk{$_}++;
                    }
                }
...
sub SortPlaylist {
    ...
    if ( $GNUpod::iTunesDB::SPLREDEF{field}{ lc($sortby) } ) {
        ...
        $sortsub = sub { uc($b) cmp uc($a) }
          if $reverse;                           #Reversed String
    }
    ...
    foreach my $cmval ( keys( %{ $self->{FuzzyDb_Lowercase}->{$sortby} } ) ) {
        foreach (
            split( / /, $self->{FuzzyDb_Lowercase}->{$sortby}->{$cmval} ) )
        {
