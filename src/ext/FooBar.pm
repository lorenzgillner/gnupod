...
sub connect {
    ...
    if ( -d $opth->{mount} ) {
        ...
        if ( !$rr->{_no_cstest}++ ) {
            _check_casesensitive( $rr->{mountpoint} )
              ;    #Check if somebody mounted the iPod caseSensitive
        }

        $rr->{autotest} = _check_autotest_mode( $rr->{mountpoint} );

        #Do an iTunesDB Sync if not disabled and needed
        StartItunesDBSync($rr)
          if ( !$opth->{_no_it_sync}
            && !$opth->{_no_sync}
            && ItunesDBNeedsSync($rr) );

        #Do an OTG Sync if not disabled and needed
        StartOnTheGoSync($rr)
          if ( !$opth->{_no_otg_sync}
            && !$opth->{_no_sync}
            && OnTheGoNeedsSync($rr) );
    }
    elsif ( $opth->{mount} ) {
...
sub _check_casesensitive {
    ...
    if ( open( CSTEST, ">$target/csTeSt" ) ) {
        my $inode_a =
          ( stat("$target/csTeSt") )[1];    #Get inode of just-creaded file
        my $inode_b =
          ( stat("$target/CStEsT") )[1];    #Get inode of another file..
        close(CSTEST)
          or die "FATAL: Could not close CSTEST FD ($target/csTeST) : $!\n";
        unlink("$target/csTeSt");           #Boom!

        if ( !defined($inode_b) || ( $inode_a != $inode_b ) )
        {    #Whops, different inodes? -> case sensitive fs
             #Nerv the user
            warn
              "$0: Warning: $target is mounted case sensitive, that's bad:\n";
            warn ""
              . ( " " x length($0) )
...
sub WipeShuffleStat {
    my ($rr) = @_;
    if ( -e $rr->{shufflestat} ) {
        unlink( $rr->{shufflestat} )
          || warn "Could not unlink '$rr->{shufflestat}', $!\n";
    }
}
...
sub SetOnTheGoAsInSync {
    ...
    if ( !( bsd_glob( $rr->{onthego}, GLOB_NOSORT ) )
        || unlink( bsd_glob( ( $rr->{onthego}, GLOB_NOSORT ) ) ) )
    {
        return undef;
    }
    else {
...
sub SetOnTheGoAsInvalid {
    my ($rr) = @_;
    open( OTGINVALID, ">$rr->{onthego_invalid}" )
      or die "Can't write $rr->{onthego_invalid}\n";
...
sub GetConfig {
      ...
      )
    {
        ...
        while ( my $line = <RCFILE> ) {
            ...
            elsif ( $doset->{$topic} eq "b" ) {
                $getopts->{$topic} = 1 if ( $val && $val ne "no" );
            }
        }
        close(RCFILE);
    }
