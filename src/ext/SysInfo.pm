...
sub GetDeviceInformation {
    ...
    _GrabFirewireGuid($info)
      if !defined( $info->{FirewireGuid} ) && !$args{NoDeviceSearch};
    return $info;
}
...
sub _GrabSysinfo {
    ...
    while (<SYSINFO>) {
        ...
        elsif ( $line =~ /^visibleBuildID: \S+ \(([^)]+)\)$/ ) {
            $hash->{Version} = $1;
        }
        elsif ( $line =~ /^ModelNumStr: (\S+)$/ ) { $hash->{Model} = $1; }
        elsif ( $line =~ /^FirewireGuid: 0x([A-Za-z0-9]{16})$/ ) {
            $hash->{FirewireGuid} = $1;
        }
    }
    close(SYSINFO);
...
sub _GrabFirewireGuid {
    ...
    if ( $Config{'osname'} eq "linux" ) {
        ...
        warn
"$0: iPod-GUID detection for '$Config{osname}' not implemented (yet)\n";
    }
}
...
sub __GrabFWGUID_LINUX {
    ...
    while ( my $dirent = readdir(BLOCKDIR) ) {
        ...
        open( UDEV, "-|" )
          or
          exec( "/sbin/udevadm", "info", "--name", $dirent, "--query", "env" );
        while (<UDEV>) {
