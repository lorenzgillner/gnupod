...
my $profiles = {
    'nano_4g' => [
        { height => 128, width => 128, storage_id => 1055, bpp => 16, },
        { height => 128, width => 128, storage_id => 1068, bpp => 16, },
        { height => 240, width => 240, storage_id => 1071, bpp => 16, },
        { height => 50,  width => 50,  storage_id => 1074, bpp => 16, },
        { height => 80,  width => 80,  storage_id => 1078, bpp => 16, },
        { height => 240, width => 240, storage_id => 1084, bpp => 16, },
    ],
    'nano_3g' => [
        { height => 320, width => 320, storage_id => 1060, bpp => 16, },
        { height => 128, width => 128, storage_id => 1055, bpp => 16, },
        {
            height     => 56,
            width      => 56,
    ...
    'nano' => [
        { height => 100, width => 100, storage_id => 1027, bpp => 16, },
        { height => 42,  width => 42,  storage_id => 1031, bpp => 16, },
    ],
    'video' => [
        { height => 200, width => 200, storage_id => 1029, bpp => 16, },
        { height => 100, width => 100, storage_id => 1028, bpp => 16, },
    ],
};
...
sub new {
    ...
    my $self = {
        ...
        storagecache   => {},
        mode           => MODE_UNPARSED,
        artworkdb      => $args{Connection}->{artworkdb},
        artworkdir     => $args{Connection}->{artworkdir},
    };
    bless( $self, $class );
    return $self;
}
        ...
        foreach my $id ( $self->_GetImageIds ) {
            if ( $self->GetImage($id)->{seen} == 0 ) {
                $self->_DeleteImage($id)
                  or die
                  "Failed to delete image # $id : Did not exist in db?!\n";
                $self->{db_dirty}++;
            }
        }
    }
...
sub GetNextDbid {
    my ($self) = @_;
    my $dbid =
      GNUpod::Ugly64->new( $self->{last_dbid_seen} )->Increment->GetHex;
...
sub _RegisterDbid {
    my ( $self, $dbid ) = @_;
    $self->{last_dbid_seen} = $dbid
      unless GNUpod::Ugly64->new($dbid)
      ->ThisIsBigger( $self->{last_dbid_seen} );
...
sub InjectImage {
    my ($self) = @_;
    Carp::confess("InjectImage($self) called in wrong mode: $self->{mode}")
      if $self->{mode} != MODE_PARSED;
    my $imgid = $self->GetNextDbid;    # Get next, free id
    $self->_RegisterNewImage(
        ref => {
            id          => 0,
            dbid        => $imgid,
            source_size => $self->{fbimg}->{source_size}
        }
    );
    ...
    foreach my $fbimg ( @{ $self->{fbimg}->{cache} } ) {
        my $dbinfo = undef;
        if ( SHARED_STORAGE
            && defined( $self->{storagecache}->{ $fbimg->{storage_id} } ) )
        {
            $dbinfo = $self->{storagecache}->{ $fbimg->{storage_id} };
        }
        else {
            $dbinfo = $self->_WriteImageToDatabase(
                Data      => $fbimg->{data},
                StorageId => $fbimg->{storage_id}
            );
            $self->{storagecache}->{ $fbimg->{storage_id} } = $dbinfo;
        }
        $self->_RegisterSubImage(
            storage_id => $fbimg->{storage_id},
            imgsize    => $fbimg->{imgsize},
            path       => ':' . $dbinfo->{filename},
            offset     => $dbinfo->{start},
            height     => $fbimg->{height},
            width      => $fbimg->{width}
        );
...
sub PrepareImage {
    my ( $self, %args ) = @_;
    Carp::confess("PrepareImage($self) called in wrong mode: $self->{mode}")
      if $self->{mode} != MODE_UNPARSED;
    $args{bgcolor} = "white" unless ( $args{bgcolor} );
    my $file  = $args{File};
    my $model = lc( $args{Model} );
    $model =~ tr/a-z0-9_//cd;    # relax
    my $mode = $profiles->{$model}
      || $profiles->{video};     # select model or use the default (video)
    my $count = 0;
    ...
    foreach my $mr (@$mode) {
        ...
        open( IM, "-|" ) || exec(
            "magick",      $file,
            "-resize",     "$mr->{height}x$mr->{width}",
            "-background", $args{bgcolor},
            "-gravity",    "center",
            "-extent",     "$mr->{height}x$mr->{width}",
            "-depth",      "8",
            "RGB:-"
        );
        binmode(IM);
        ...
        my $size =
          ( $mr->{height} * $mr->{width} * $mr->{bpp} / 8 ) - $mr->{drop};
        my $rgb565 = substr( $conv->RGB888ToRGB565, 0, $size );
        my $outlen = length($rgb565);

        if ( $size != $outlen ) {
            warn
"$0: Could not convert $file to $mr->{height}x$mr->{width}: image should be $size bytes but imagemagick provided $outlen bytes.\n";
            next;
        }
        push(
            @{ $self->{fbimg}->{cache} },
...
sub _WriteImageToDatabase {
    my ( $self, %args ) = @_;
    Carp::confess(
        "_WriteImageToDatabase($self) called in wrong mode: $self->{mode}")
      if $self->{mode} != MODE_PARSED;
    ...
    my $start =
      ( $self->{storages}->{ $args{StorageId} }->{last_offset_used} || 0 )
      ;                   # Offset we are going to write
    my $i = ( $self->{storages}->{ $args{StorageId} }->{last_index_used} || 1 )
      ;                   # Image-Id index (F????_X.ithmb)
    ...
    for ( ; ; $start += $len ) {
        ...
        if ( $self->{storages}->{ $args{StorageId} }->{ithmb}->{ ":" . $fnam }
            ->{$start} == 0 )
        {
...
sub WriteArtworkDb {
    ...
    Carp::confess("WriteArtworkDb($self) called in wrong mode: $self->{mode}")
      if $self->{mode} != MODE_PARSED;
    ...
    print $fd GNUpod::iTunesDB::mk_mhxx(
        { childs => int( $self->_GetImageIds ), name => 'mhli' } );
    foreach my $imgid ( $self->_GetImageIds ) {
        ...
        foreach my $subref (@$subimgs) {
            $subref->{payload} = GNUpod::iTunesDB::mk_awdb_mhod(
                { type => 0x03, payload => $subref->{path} } );
            $subref->{childs} = 1;    # We will always write one child
            $mhii_child_payload .= GNUpod::iTunesDB::mk_awdb_mhod(
                { type => 0x02, payload => GNUpod::iTunesDB::mk_mhni($subref) }
            );
        }
        print $fd GNUpod::iTunesDB::mk_mhii(
            {
                dbid        => $imgobj->{dbid},
                childs      => int(@$subimgs),
                payload     => $mhii_child_payload,
                id          => $imgobj->{id},
                rating      => $imgobj->{rating},
                source_size => $imgobj->{source_size}
            }
        );
    }
    ...
    my $fake_mhla =
      GNUpod::iTunesDB::mk_mhxx( { childs => 0, name => 'mhla' } );
    print $fd GNUpod::iTunesDB::mk_mhsd(
        { type => 0x02, size => length($fake_mhla) } );
    print $fd $fake_mhla;
    ...
    print $fd GNUpod::iTunesDB::mk_mhxx(
        { childs => int( $self->GetStorageIds ), name => 'mhlf' } );
    foreach my $stid ( $self->GetStorageIds ) {
        print $fd GNUpod::iTunesDB::mk_mhif(
            {
                childs  => 0,
                payload => '',
                id      => $stid,
                imgsize => $self->GetStorage($stid)->{imgsize}
            }
        );
    }
    ...
    GNUpod::FooBar::SeekFix( $fd, $mhsd_mhif_fixup,
        GNUpod::iTunesDB::mk_mhsd( { type => 0x03, size => $mhsd_mhif_size } )
    );
    GNUpod::FooBar::SeekFix( $fd, $mhsd_mhii_fixup,
        GNUpod::iTunesDB::mk_mhsd( { type => 0x01, size => $mhsd_mhii_size } )
    );
    GNUpod::FooBar::SeekFix(
        $fd, 0,
        GNUpod::iTunesDB::mk_mhfd(
            {
                next_id => $self->{last_id_seen} + 1,
                childs  => 0x03,
                size    => $mhfd_size
            }
        )
...
sub _RegisterNewImage {
    ...
    my %h = %{ $args{ref} };
    Carp::confess(
        "_RegisterNewImage($self) called in wrong mode: $self->{mode}")
      if $self->{mode} == MODE_UNPARSED;
    $self->{ctx} = undef;

    if ( $self->{images}->{ $h{dbid} } ) {
        warn
"$0: $h{dbid} is registered, looks like your ArtworkDB is corrupted?!\n";
    }
    else {
        ...
        $self->{last_id_seen} = $h{id}
          if $self->{last_id_seen} < $h{id};    # Remember latest id we saw
...
sub RegisterStorage {
    ...
    unless ( exists $self->{storages}->{ $args{storage_id} } ) {
        $self->{storages}->{ $args{storage_id} } =
          { ithmb => {}, imgsize => $args{imgsize} };
    }

    my $itr = $self->{storages}->{ $args{storage_id} }->{ithmb};
    $itr->{ $args{path} }->{ $args{used} } +=
      1;    # Mark this beginning block as used
...
sub GetHex {
    my ($self) = @_;
    return
        unpack( "H8", pack( "V", $self->{num}->[0] ) )
      . unpack( "H8", pack( "V", $self->{num}->[1] ) );
}
...
sub RGB888ToRGB565 {
    ...
    for ( my $h = 0 ; $h < $self->{dimH} ; $h++ ) {
        for ( my $w = 0 ; $w < $self->{dimW} ; $w++ ) {
            ...
            for ( my $i = 0 ; $i < $bpp ; $i++ ) {
                $A[$i] =
                  unpack( "C", substr( $self->{data}, $offset + $i, 1 ) );
            }
