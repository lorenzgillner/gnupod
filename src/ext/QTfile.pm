...
########### Now we build the chain #######################################
    ...
    my $video_index =
      get_media_index( $lx{metadat}{'::moov::trak::mdia::hdlr'}, VIDEO_ITEM );
    my $sound_index =
      get_media_index( $lx{metadat}{'::moov::trak::mdia::hdlr'}, SOUND_ITEM );
    ...
    my @METADEF = (
        "album",   "\xA9alb", "comment",  "\xA9cmt", "genre",  "\xA9gen",
        "group",   "\xA9grp", "composer", "\xA9wrt", "artist", "\xA9ART",
        "title",   "\xA9nam", "fdesc",    "\xA9too", "year",   "\xA9day",
        "comment", "\xA9cmt"
    );
...
###INT and such fields are here:
    ...
    if ( my $cDat =
        $lx{metadat}{'::moov::udta::meta::ilst::tmpo::data'}[$sound_index] )
    {
        $reth{bpm} = GNUpod::FooBar::shx2_x86_int($cDat);
    }

    if ( my $cDat =
        $lx{metadat}{'::moov::udta::meta::ilst::trkn::data'}[$sound_index] )
    {
    ...
    if ( my $cDat =
        $lx{metadat}{'::moov::udta::meta::ilst::disk::data'}[$sound_index] )
    {
    ...
    foreach my $cidentify ( ( $sound_index, $video_index ) ) {
        ...
        if (
            my $cDat = (
                     $lx{metadat}{'::moov::mvhd'}[$cidentify]
                  or $lx{metadat}{'::moov::trak::mdia::mdhd'}[$cidentify]
            )
          )
        {
            #Calculate the time...
            $reth{time} =
              int( get_string_oct( 8, 4, $cDat ) /
                  get_string_oct( 4, 4, $cDat ) *
                  1000 );
            last if $reth{time};
        }
    }

    if ( $lx{metadat}{'::moov::udta::meta::ilst::----::mean'}[$sound_index] eq
        "apple.iTunes"
        && $lx{metadat}{'::moov::udta::meta::ilst::----::name'}[$sound_index]
        eq "NORM" )
    {
        $reth{iTunNORM} =
          $lx{metadat}{'::moov::udta::meta::ilst::----::data'}[$sound_index];
    }

    if ( my $cDat =
        $lx{metadat}{'::moov::trak::mdia::minf::stbl::stsd'}[$sound_index] )
    {
    ...
    if ( !$reth{genre} ) {
        my $numeric_genre = GNUpod::FooBar::shx2_x86_int(
            $lx{metadat}{'::moov::udta::meta::ilst::gnre'}[$sound_index] );
        $reth{genre} = "(" . ( $numeric_genre - 1 ) . ")"
          if $numeric_genre > 0;    # Blues = 1 ; but blues == 0 on id3tags
    }
    ...
    $reth{bitrate} =
      int( ( $reth{filesize} * 8 / 1024 ) / ( 1 + $reth{time} ) * 1000 );
...
sub get_atom {
    ...
    if ( defined( $hchild{$typ} ) ) {    #This type has a child
                                         #Track the old level
        $lt->{ltrack}->{ $pos + $len } = $level
          unless $lt->{ltrack}->{ $pos + $len };
    ...
    elsif ( $len >= 16 && $cChain !~ /(::mdat|::free)$/ )
    {    #No child -> final element -> data!
...
sub rseek {
    ...
    if ( $len < 0 ) {
        warn
"QTFile.pm: rseek($offset,$len) : POSSIBLE BUG OR BROKEN FILE: Calling rseek with \$len == 0 isn't such a good idea?!\n";
    }
    elsif ( $len > MAX_RSEEK_DATA ) {
        warn
"QTFile.pm: rseek($offset,$len) : POSSIBLE BUG OR BROKEN FILE: Ouch! rseek refuses to return $len bytes; Request cropped to "
          . MAX_RSEEK_DATA . "\n";
        $len = MAX_RSEEK_DATA;
    }
