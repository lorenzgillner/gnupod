...
sub getpath {
    ...
    if ( $opts->{keepfile} ) {
        ...
        my $clean_filename =
          ( split( /\//, $source ) )[-1];    # Extracts the filename
        my ($current_extension) =
          $clean_filename =~ /\.([^.]*)$/;    # Current file extension
        $clean_filename =~ tr/a-zA-Z0-9/_/c;  # Remove bad chars
        my $requested_ext = ( $opts->{extension} || $opts->{format} )
          ;    # Checks if existing extension matches regexp
        my @aviable_targets = bsd_glob( $xconn->{musicdir} . "/*",
            $xconn->{autotest} ? GLOB_NOCASE : GLOB_NOSORT );

        unless ( int(@aviable_targets) ) {
            warn
"No iPod folders found at $xconn->{mountpoint}, did you run gnupod-init ?\n";
            return undef;
        }
        if ( length($current_extension) != 0 ) {

            # -> Removes extension
            $clean_filename = substr( $clean_filename, 0,
                ( 1 + length($current_extension) ) * -1 );
        }
        if (   $opts->{format}
            && $requested_ext
            && $current_extension !~ /$requested_ext/i )
        {
            if ( length($current_extension) != 0 ) {

                # Only warn if file HAD an extension before
                warn
"Warning: $source has a wrong extension [$current_extension], changed extension to $opts->{format}\n";
            }
        ...
        for ( my $i = 0 ; $i < MAX_PATHLOOP ; $i++ ) {
            ...
            my $dp_target =
              $aviable_targets[ $xconn->{autotest}
              ? 0
              : int( rand(@aviable_targets) ) ];
            my $dp_path = $dp_target . "/" . $dp_prefix;
            my $dp_chrleft =
              MAX_PATHLENGTH -
              length($dp_path) +
              length( $xconn->{mountpoint} );
            my $dp_ext    = ".$current_extension";
            my $dp_extlen = length($dp_ext);
            next
              if $dp_chrleft <
              $dp_extlen;    # No space for extension, checkout something else
            $dp_path .=
              substr( $clean_filename, ( $dp_chrleft - $dp_extlen ) * -1 )
              . $dp_ext;

            if ( !( -e $dp_path ) && open( TEST, ">", $dp_path ) ) {
                close(TEST);
                unlink($dp_path)
                  or die "FATAL: Unable to unlink $dp_path : $!\n";
                $destination = $dp_path;
                last;
            }
        }
...
sub xescaped {
    ...
    return $ret
      unless ( $ret =~ /[^\x20-\x7F]/ )
      ;    #early return if nothing outside plain ascii range
    ...
    # BOM removal
    $ret =~ s/\xEF\xBB\xBF//g
      ;    # real BOM got converted to a "utf8.BOM" and isn't needed anymore
           # strip other control characters
    $ret =~ tr/\000-\037//d;

    #convert to XML encoded unicode
    $ret =~
s/([\xC2-\xDF])([\x80-\xBF])/"&#x".sprintf("%x", ((ord($1) & 31) <<  6) +  (ord($2) & 63) ).";"/eg;
    $ret =~
s/([\xE0-\xEF])([\x80-\xBF])([\x80-\xBF])/"&#x".sprintf("%x", ((ord($1) & 15) << 12) + ((ord($2) & 63) <<  6) +  (ord($3) & 63) ).";"/eg;
    $ret =~
s/([\xF0-\xF4])([\x80-\xBF])([\x80-\xBF])([\x80-\xBF])/"&#x".sprintf("%x", ((ord($1) &  7) << 18) + ((ord($2) & 63) << 12) + ((ord($3) & 63) <<  6) + (ord($4) & 63) ).";"/eg;
    ...
    if ( $in ne $ret ) {
        use Data::Dumper;
        print
"Something fishy was removed from your XML data. Here's the before/after data:\n";
        print Dumper($in);
        print Dumper($ret);
    }
    return $ret;
...
sub addpl {
    ...
    if ( ref( $XDAT->{playlists}->{pref}->{$name} ) eq "HASH" ) {
        warn
          "XMLhelper.pm: No need to create '$name', playlist exists already!\n";
        return;
    }
...
sub addspl {
    ...
    if ( ref( $XDAT->{spls}->{pref}->{$name} ) eq "HASH" ) {
        warn
"XMLhelper.pm: No need to create '$name', smartplaylist exists already!\n";
        return;
    }
...
sub getpl_content {
    my ($plname) = @_;
    return @{ $XDAT->{playlists}->{data}->{$plname} }
      if ref( $XDAT->{playlists}->{data}->{$plname} ) eq "ARRAY";
...
sub eventer {
    ...
    if ( $href->{Context}[1] eq "files" ) {
        ...
        warn
"** XMLhelper: Found <file ../> item *after* a <playlist ..>, that's bad\n"
          if getpl_attribs();
...
sub mkh {
    ...
    for ( my $i = 0 ; $i < int(@content) ; $i += 2 ) {
        $href->{$base}->{ $content[$i] } =
          Unicode::String::utf8( $content[ $i + 1 ] )->utf8;
    }
    return $href;
}
    ...
    foreach ( getpl_attribs() ) {
        ...
        if ( my $shr = get_splpref($current_plname) ) {    #xmlheader present
            ...
            if ( $pl && @{$pl} ) {    # the playlist is not empty
                ...
                warn
                  "XMLhelper.pm: Dropping empty playlist '$current_plname'!\n";
            }
        }
        else {
            warn
"XMLhelper.pm: bug found: unhandled plitem $_ inside $current_plname\n";
        }
    }
    ...
    if ( close(OUT) ) {
        if ( -e $out ) {                           #Backup old out file
            rename( $out, $out . ".old" )
              or warn "Could not move $out to $out.old\n";
        }
