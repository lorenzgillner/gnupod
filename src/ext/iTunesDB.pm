...
sub mk_itunes_sd_file {
    ...
    $ret .=
      tnp( $ref->{soundcheck} == 0
        ? 0
        : int( log( $ref->{soundcheck} / 1000 ) / log(10) / -0.1 ) );
    ...
    $bmflag += 0x000100
      if $ref->{bookmarkable} != 0;   #Well.. this is somehow broken.. hmm FIXME
...
sub mk_mhit {
    ...
    if    ( $vol >= 0 && $vol <= 255 ) { }    #Nothing to do
        ...
        warn
"** Warning: ID $file_hash{id} has volume set to $file_hash{volume} percent. Volume set to +-0%\n";
    ...
    foreach ( ( "rating", "prerating" ) ) {
        if ( $file_hash{$_} && $file_hash{$_} !~ /^(2|4|6|8|10)0$/ ) {
            warn
"Warning: Song $file_hash{id} has an invalid $_: $file_hash{$_}\n";
            $file_hash{$_} = 0;
        }
    }
    ...
    # Convert legacy dbid :-(
    if (  !defined( $file_hash{dbid_1} )
        && defined( $file_hash{dbid_msw} )
        && defined( $file_hash{dbid_lsw} ) )
    {
        $file_hash{dbid_1} = unpack( "H16",
                pack( "V", $file_hash{dbid_lsw} )
              . pack( "V", $file_hash{dbid_msw} ) );
    }
    ...
    $ret .=
      pack( "v", 0x100 );   #cbr = 100, vbr = 101, aac = 0x010 ##FIXME: COrrect?
    ...
    $ret .=
      pack( "H16", ( $file_hash{dbid_1} ) );    # First dbid, should not be null
    $ret .= pack("v");                                           #??
...
##   - type id
    ...
    if ( $type == 7 && $string !~ /#!#\d+#!#/ ) {
        ...
        warn
          "             value should be like '#!#NUMBER#!#', ignoring value\n";
        $string = undef;
    }
    ###
...
# ..-> iTunes does the same :)
        push(
            @{ $hs->{data} },
            { field => 'ARTIST', action => 'CONTAINS', string => "" }
        );
    }
    ...
    foreach my $chr ( @{ $hs->{data} } ) {
        ...
        {    #String type
            ...
            unless ( $action_num =
                $SPLREDEF{string_action}{ uc($real_action) } )
            {
                warn
"iTunesDB.pm: action $chr->{action} is invalid for $chr->{field} , skipping rule\n";
                next;
            }

        }
        elsif ( $int_field = $SPLREDEF{field}{ lc( $chr->{field} ) } )
        {    #Int type
...
"iTunesDB.pm: action $chr->{action} is invalid for $chr->{field}, skipping rule\n";
            ...
            my ( $within_magic_a, $within_magic_b, $within_range, $within_key )
              = undef;
            ...
            if ( $action_num == $SPLREDEF{num_action}{within} ) {
                ...
                $within_magic_b =
                  0xffffffff;    #Isn't magic.. but we are not 64 bit..
                ( $within_range, $within_key ) =
                  $chr->{string} =~ /(\d+)_(\S+)/;

                if ( $SPLREDEF{within_key}{ lc($within_key) } ) {
                    $within_key =
                      $SPLREDEF{within_key}{ lc($within_key) };    #Known
                }
                else {
                    warn
"Invalid value for 'within' action: '$chr->{string}', skipping rule\n";
                    next;
                }
            ...
            $string .= pack( "N", _icl( $within_magic_b - $within_range ) )
              ;    #0-0 for non within
            $string .= pack("N");
            ...
            warn
"iTunesDB.pm: ERROR: <spl field=\"$chr->{field}\"... is unknown, skipping SPL\n";
            next;
        }

        if ( length($string) > 0xfe ) {    #length field is limited to 0xfe!
            warn
"iTunesDB.pm: splstring to long for iTunes, cropping (yes, that's stupid)\n";
            $string = substr( $string, 0, 254 );
        }

        $cr .= pack("H6");    #Add data in for() loop... (= new chunk)
        $cr .= pack( "C", _icl( $int_field, 0xff ) );
        $cr .= pack( "N", _icl( $action_num + $action_prefix ) )
          ;                   #Yepp.. everything here is x86! ouch
        $cr .= pack("H94");
    ...
    return undef
      unless $CHTM;    #Ouch, EVERYTHING failed. Refuse to create an empty SPL
    ...
    $ret .= pack("h6");
    $ret .= pack( "C", _icl( $CHTM, 0xff ) )
      ;    #HTM (Childs from cr) FIXME: is this really limited to 0xff childs?
    $ret .= pack("h6");
...
sub mk_mhyp {
    ...
    $ret .= pack( "V",  0 )
      ;    #_icl($PLREDEF{sort}{$hr->{sortflag}})); Fixme: is this even used?
...
sub mk_awdb_mhod {
    ...
    my $ret = 'mhod';
    $ret .= pack( "V", _icl($size_header) );    # Size of header
    $ret .= pack( "V", _icl($size_mhod) )
      ;    # Size of header + 2 unknown items + size of payload
...
sub mk_mhni {
    ...
    my $size_mhni =
      $size_header + length( $hr->{payload} );    # Size of mhni + append
...
sub _icl {
    ...
    if ( $int > $checkmax or $int < 0 ) {
        _itBUG(
"_icl: Value '$int' is out of range! (Maximum: $checkmax)\n => Forcing value to $checkmax\
...
sub get_int {
    ...
    Carp::confess(
        "Short read: Wanted $toread bytes, got $bytes_read, syserr: $!\n")
      if $toread != $bytes_read;
    return
      int( unpack( "V", pack( "H16", unpack( "H16", $buff ) ) ) )
      ;    # inlined shx2int
}
...
sub read_spldata {
    ...
    my $diff =
      $hr->{start} + 160;  # Hardcoded start offset (we are still inside a mhod)
    my @ret = ();
    ...
    for ( 1 .. $hr->{htm} ) {
        ...
        my $slen   = get_int( $diff + 55, 1, $fd )
          ; #Whoa! This is true: string is limited to 0xfe (254) chars!! (iTunes4)
        ...
        if ( $SPLDEF{is_string}{$ftype} ) {          #Is a string type
            ...
            #Warn about bugs
            $SPLDEF{string_action}{$action}
              or _itBUG(
"Unknown s_action $action for $ftype (= GNUpod doesn't understand this SmartPlaylist)"
              );
        }
            ...
            && get_x86_int( $diff + 56,     4, $fd ) == 0x2dae2dae )
        {
            ...
            my $within_key =
              $SPLDEF{within_key}{ get_x86_int( $diff + 56 + 20, 4, $fd ) }
              ;                                            #Set within key
            if ($within_key) {
                $rs = $rs . "_" . $within_key;
            }
                ...
                _itBUG(
"Can't handle within_SPL_FIELD - unknown within_key, using 1_day"
                );
            ...
            $SPLDEF{num_action}{$action}
              or _itBUG(
"Unknown num_action $action for $ftype (= GNUpod doesn't understand this SmartPlaylist)"
              );
        }
        ...
        $SPLDEF{field}{$field}
          or _itBUG(
"Unknown SPL-Field: $field (= GNUpod doesn't understand this SmartPlaylist)"
          );
        push( @ret,
            { action => $human_exp, field => $human_field, string => $rs } );
    }
...
#$sort value ;)
    ...
    if ( $SPLDEF{limitsort}{$sort} ) {
        ...
        _itBUG(
            "Don't know how to handle SPLSORT '$sort', setting sort to RANDOM",
        );
        $sort = "random";
    }

    $SPLDEF{limititem}{ int($item) } or warn "Bug: limititem $item unknown\n";
    $SPLDEF{checkrule}{ int( $chklim + ( $chkrgx * 2 ) ) }
      or warn "Bug: Checkrule "
      . int( $chklim + ( $chkrgx * 2 ) )
      . " unknown\n";
    return (
        {
            live      => $live,
            value     => $limit,
            iitem     => $SPLDEF{limititem}{ int($item) },
            isort     => $sort,
            mos       => $mos,
            checkrule => $SPLDEF{checkrule}{ int( $chklim + ( $chkrgx * 2 ) ) }
        }
    );
}
...
sub get_awdb_mhod {
    ...
    if ( $r{type} == 2 ) {
        ...
        my $tmpstring =
          get_string( $seek + $r{total_size} - $strlen, $strlen, $fd );
...
sub get_mhod {
    ...
    if ( $r{type} == 15 or $r{type} == 16 ) {
       ...
       # Strings are the same as < 15 mhods but utf8 encoded... .. ........ why?
        my $tmpstring = get_string( $seek + $r{header_size},
            $r{total_size} - $r{header_size}, $fd );
        ...
        $r{splpref} =
          read_splpref( { start => $seek, end => $r{total_size} }, $fd );
    }
    elsif ( $r{type} == 51 ) {
        $r{matchrule} = get_int( $seek + 39, 1, $fd );
        $r{spldata}   = read_spldata(
            { start => $seek, htm => get_int( $seek + 35, 1, $fd ) }, $fd );
    }
    elsif ( $r{type} == 100 ) {
        ...
        my $tmpstring =
          get_string( $seek + ( $r{total_size} - $r{string_size} ),
            $r{string_size}, $fd );
        $tmpstring = Unicode::String::byteswap2($tmpstring);
        $r{string} = Unicode::String::utf16($tmpstring)->utf8;
    }
...
sub get_string {
    ...
    Carp::confess(
        "Short read: Wanted $anz bytes at $start, got $bytes_read, syserr: $!\n"
    ) if $anz != $bytes_read;
    return $buffer;
}
...
sub get_mhit {
    ...
    $o{playcount}   = get_int( $offset + 80, 4, $fd )
      ; #84 has also something to do with playcounts. (Like rating + prerating?)
    ...
    if ( $r{header_size} >= NEW_ITUNESDB_MHIT_HEADERSIZE ) {
        ...
        $o{has_artwork} = 1
          if get_int( $offset + 164, 1, $fd ) ==
          1;    # 1 = Has artwork ; 2 = No artwork ; 0 = undef?
...
sub _Real_ParseiTunesDB {
    ...
    foreach my $this_child ( 1 .. $this_childs ) {
        ...
        if ( $t eq 'mhod' && $obj->{awdb} ) {
            $r = get_awdb_mhod( $obj->{offset}, $obj->{fd} );
        }    # ArtworkDB mhods are different :-/
        elsif ( $t eq 'mhod' ) {
            $r = get_mhod( $obj->{offset}, $obj->{fd} );
            ( $r->{header_size} = $r->{total_size} );
        }    # Don't care about the string
        elsif ( $t eq 'mhni' ) { $r = get_mhni( $obj->{offset}, $obj->{fd} ) }
        elsif ( $t eq 'mhit' ) { $r = get_mhit( $obj->{offset}, $obj->{fd} ) }
        elsif ( $t eq 'mhyp' ) {
            $r = get_mhyp( $obj->{offset}, $obj->{fd} );
            $obj->{xhack} = $r->{childs};
            $r->{childs} += $r->{scount};
        }
        ...
        elsif ( $t =~ /^mh(lt|lp|la|lf|li)$/ ) {
            $r = get_mhxx( $obj->{offset}, $obj->{fd} );
        }
        else {
        ...
        if ( $next_atom == $obj->{offset} ) {
            ...
            if (
                defined(
                    my $cb =
                      $obj->{callback}->{ $obj->{tree}->{ $deep - 1 }->{name} }
                      ->{start}
                )
              )
            {
                $obj->{callback}->{PACKAGE}->$cb(
                    offset => $obj->{offset},
                    ref    => $obj->{tree}->{ $deep - 1 }->{ref}
                );
            }
        }
        elsif ( $ft ne $t ) {
            die sprintf(
"Walking error at %08X , found type $t but expected to find $ft\n",
                $obj->{offset} );
        }

        if ( defined( my $cb = $obj->{callback}->{$t}->{item} ) ) {
            $obj->{callback}->{PACKAGE}
              ->$cb( offset => $obj->{offset}, ref => $r );
        }

        if (ITUNES_DUMP) {
            print " " x ( $deep * 3 );
            printf(
"%02d Atom=>%s, Offset=>%04X, ThisCount=>%d, TotalCount=>%d, NextAtom=>%04X, NextChilds=>%d, Type=>%d\n",
                $deep, $t, $obj->{offset}, $this_child, $this_childs,
                $next_atom, $r->{childs}, $r->{type} );
        }

        if (   defined( $obj->{xhack} )
            && $t eq 'mhod'
            && $this_child == $obj->{xhack} )
        {
    ...
    if (
        defined(
            my $cb =
              $obj->{callback}->{ $obj->{tree}->{ $deep - 1 }->{name} }->{end}
        )
      )
    {
        $obj->{callback}->{PACKAGE}->$cb(
...
sub readPLC {
    ...
    my $chunksize = get_int( 8, 4, *PLC )
      ; #How long is one entry? (20 for iTunes 0xD / 16 for V2 Firmware, 12 for v1)
    ...
    for my $chunknum ( 1 .. $chunks ) {
        $pcrh{playcount}{$chunknum} = get_int( $offset + 0, 4, *PLC )
          if $chunksize >= 4;
        $pcrh{lastplay}{$chunknum} = get_int( $offset + 4, 4, *PLC )
          if $chunksize >= 8;
        $pcrh{bookmark}{$chunknum} = get_int( $offset + 8, 4, *PLC )
          if $chunksize >= 12;
        $pcrh{rating}{$chunknum} = get_int( $offset + 12, 4, *PLC )
          if $chunksize >= 16;
        $pcrh{skipcount}{$chunknum} = get_int( $offset + 20, 4, *PLC )
          if $chunksize >= 24;
        $pcrh{lastskip}{$chunknum} = get_int( $offset + 24, 4, *PLC )
          if $chunksize >= 28;
...
sub readOTG {
    ...
    foreach my $file ( bsd_glob( $glob, GLOB_NOSORT ) ) {
        ...
        for ( 1 .. $items ) {
            ...
            if ( $rb == 0 ) {
                _itBUG( "readOTG($glob) i:$items / f:$file / s:$offset / fs: "
                      . ( -s $file ) );
                last;
            }
