...
sub go {
    ...
    print "  Total playtime : "
      . int( $TRACKER{TIME} / 1000 / 60 / 60 ) . " h\n";
    printf( "  Total filesize : %.2f GB",
        ( $TRACKER{SIZE} / 1024 / 1024 / 1024 ) );
    print " ($TRACKER{SIZE} Bytes) \n";
    ...
    if ( $opts{fixit} ) {
        if ( $TRACKER{FIXED} ) {
            print
" -> Writing new GNUtunesDB.xml, i fixed $TRACKER{FIXED} Errors.\n";
        ...
        if ( $TRACKER{GLOBFILES} == $TRACKER{ITFILES} && $TRACKER{ERR} == 0 ) {
            ...
            print
" -> The GNUtunesDB.xml is inconsistent. Please try to fix the errors or run $0 --fixit\n";
...
sub checkGNUtunes {
    ...
    foreach my $file ( bsd_glob( $c->{musicdir} . "/*/*", GLOB_NOSORT ) ) {
        next if -d $file;
        $TRACKER{GLOBFILES}++;
        unless ( $TRACKER{PATH}{ lc($file) } )
        {    #Hmpf.. file maybe not in the GNUtunesDB
            print
"  Stale file '$file' found. Remove the file, it wastes space...\n";
            $TRACKER{ERR}++;
            if ( $opts{fixit} ) {
...
sub newfile {
    ...
    $TRACKER{ID}{ int($id) }++;
    $TRACKER{PATH}{ lc($rp) }++
      ; #FAT32 is caseInsensitive.. HFS+ should also be caseInsensitive (ON THE IPOD)
    $TRACKER{ITFILES}++;

    if ( $TRACKER{ID}{ int($id) } != 1 ) {
        print "  ID $id is used "
          . int( $TRACKER{ID}{ int($id) } )
          . " times!\n";
        $TRACKER{ERR}++;
        if ( $opts{fixit} ) {
            print
" fixit: Removing this file from XML, re-run $0 to get rid of the stale file!\n";
            $call_mkfile = undef;
            $TRACKER{FIXED}++;
        }
    }
...
" fixit: Removing this file from XML, re-run $0 to get rid of the stale file!\n";
    ...
    if ( length( $el->{file}->{path} ) > $xmlhelper_maxpathlen ) {
        print
"  ID $id has a long filename. Some iPods may refuse to play this file with recent firmware! Run $0 --fixit\n";
        $TRACKER{ERR}++;
        if ( $opts{fixit} ) {
            my ( $ipod_path, $real_path ) =
              GNUpod::XMLhelper::getpath( $con, $rp );
            if ($ipod_path) {
                ...
                print
" fixit: Ouch, no new path found for $rp, removing from xml\n";
                $call_mkfile = undef;
            }
            $TRACKER{FIXED}++;
        }
    ...
    if ( !-e $rp ) {
        ...
        if ( $opts{fixit} ) {
            print
" fixit: Removing $id from XML Database, please re-run $0 to get rid of the stale file!\n";
            $call_mkfile = undef;
            $TRACKER{FIXED}++;
        }
    }
