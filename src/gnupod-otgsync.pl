...
GNUpod::FooBar::GetConfig( \%opts,
    { nosync => 'b', 'automktunes' => 'b', model => 's' }, "otgsync" );
...
"gnupod-otgsync.pl: Bug detected! You need to run tunes2pod.pl -> Sync broken!\n";
    ...
    if ( GNUpod::FooBar::OnTheGoDataIsInvalid($con) )
    {                         #Ok, On-The-Go data is ** BROKEN **
...
sub mkotg {
    ...
    foreach (@otgrefs) {
        ...
        foreach (@xotg) {
            ...
            GNUpod::XMLhelper::mkfile( $plfh,
                { "plname" => "On-The-Go $otggen" } );
        }
        $otggen++;
    }
...
sub newfile {
    ...
    if ($plcref) {    #PlayCountref exists (=v2 ipod) -> adjust
        ...
        $el->{file}->{played_flag} = 1
          if ( $el->{file}->{playcount} > 0
            || $el->{file}->{skipcount} > 0
            || $el->{file}->{bookmark} > 0 );
