...
C<tunes2pod> to update the (outdated) GNUtunesDB.
...
you would lose any changes you made with iTunes. Running C<tunes2pod>
...
=head2 Not running mktunes

If you unmount your iPod without running C<mktunes> then you'll find
...
C<mktunes> and unmount to recover from this.
...
In this case, you can use C<gnupod-check> to search 'lost' files.
gnupod-check will tell you what you could do to fix it.

If gnupod-check found many errors (or you lost your
F<GNUtunesDB.xml>), you can 'rebuild' an GNUtunesDB using:

     gnupod-addsong --restore -m /mnt/ipod
...
yours.  Then run C<mktunes>.  This might be sufficient to return your
